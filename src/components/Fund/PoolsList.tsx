      ...
      <table className="Exchange-list large App-box">
        <tbody>
          ...
          {pools.length === 0 && !poolDataIsLoading && (
            <tr>
              <td colSpan={15}>
                <div className="Exchange-empty-positions-list-note">No open pools</div>
              </td>
            </tr>
          )}
          {pools.map((pool) => {
              ...
              <tr key={pool.key}>
                <td className="clickable">
                  <div className="Exchange-list-title">
                    ...
                    {pool.hasPendingChanges && <ImSpinner2 className="spin position-loading-icon" />}
                  </div>
                </td>
                <td>
                  <div>{pool.vAPY}</div>
                ...
                <td>
                  <div>${formatAmount(pool.tvl, USD_DECIMALS, 2, true)}</div>
                </td>
                <td>
                  <button className="Exchange-list-action" onClick={() => closePool(pool)} disabled={false}>
                    Close
                  </button>
                </td>
              </tr>
